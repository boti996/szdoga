...
# convolution block: convolution + batch normalization + leaky ReLU
...
# residual block: top and left padding + 3x3 convolution blocks with n/2 bottleneck + residual value
...
# darknet feature extractor: 1 + 3 + 4 + 17 + 17 + 9 = 52 convolution layers from residual blocks
def darknet_body(x):
...
# output features from various scales: convolution blocks with bottlenecks
...
# feature extractor + concatenated features from various scales + final output layer
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    anchors_tensor = keras.reshape(keras.constant(anchors), [1, 1, 1, num_anchors, 2])  # anchor widths and heights

    grid_shape = keras.shape(feats)[1:3]  # height, width
    grid_y = keras.tile(keras.reshape(keras.arange(0, stop=grid_shape[0]), [-1, 1, 1, 1]),
                        [1, grid_shape[1], 1, 1])
    grid_x = keras.tile(keras.reshape(keras.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]),
                        [grid_shape[0], 1, 1, 1])
    grid = keras.concatenate([grid_x, grid_y])  # a grid's width and height in [x,y] format
...
# correct box format (left upper and right lower corner + adjusted to original input shape)
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    """Get corrected boxes"""
    box_yx = box_xy[..., ::-1]
    box_hw = box_wh[..., ::-1]
    input_shape = keras.cast(input_shape, keras.dtype(box_yx))
    image_shape = keras.cast(image_shape, keras.dtype(box_yx))
    new_shape = keras.round(image_shape * keras.min(input_shape / image_shape))
    ...
    boxes *= keras.concatenate([image_shape, image_shape])
    return boxes


# boxes and box scores
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    """Process Conv layer output"""
    box_xy, box_wh, box_confidence, box_class_probs = yolo_head(feats,
                                                                anchors, num_classes, input_shape)
    boxes = yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape)
    boxes = keras.reshape(boxes, [-1, 4])
    box_scores = box_confidence * box_class_probs
    box_scores = keras.reshape(box_scores, [-1, num_classes])
...
def yolo_eval(yolo_outputs,
    ...
    input_shape = keras.shape(yolo_outputs[0])[1:3] * 32
    boxes = []
    box_scores = []
    ...
    boxes = keras.concatenate(boxes, axis=0)
    box_scores = keras.concatenate(box_scores, axis=0)

    mask = box_scores >= score_threshold
    max_boxes_tensor = keras.constant(max_boxes, dtype='int32')
    boxes_ = []
    scores_ = []
    classes_ = []
    for c in range(num_classes):
        ...
        class_boxes = keras.gather(class_boxes, nms_index)
        class_box_scores = keras.gather(class_box_scores, nms_index)
        classes = keras.ones_like(class_box_scores, 'int32') * c
        boxes_.append(class_boxes)
        scores_.append(class_box_scores)
        classes_.append(classes)
    boxes_ = keras.concatenate(boxes_, axis=0)
    scores_ = keras.concatenate(scores_, axis=0)
    classes_ = keras.concatenate(classes_, axis=0)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        _, ignore_mask = keras.control_flow_ops.while_loop(lambda b, *args: b < m, loop_body, [0, ignore_mask])
        ...
        if print_loss:
            loss = tf.Print(loss, [loss, xy_loss, wh_loss, confidence_loss, class_loss, keras.sum(ignore_mask)],
                            message='loss: ')
    return loss
